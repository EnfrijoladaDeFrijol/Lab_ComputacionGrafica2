...
int main() {
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		view = glm::translate(view, glm::vec3(0.0f,0.0f,-10.0f));					// Traslacion (,,profundidad)
		model = glm::rotate( model, 0.5f, glm::vec3( 10.0f, 5.0f, 2.0f ) );			// Rotacion (x,y,z)
		...
		// ==-==-==-==-==-==-==-==-==-==-==-==-==-==-==-==-==-==-==-==-==-==-==-==-
		// A G R E G A N D O   O T R O   C U B O
		model = glm::mat4(1);
		model = glm::translate(view, glm::vec3(10.0f, 0.0f, 0.0f));
		model = glm::rotate(model, 0.5f, glm::vec3(0.0f, 10.0f, 1.0f));
		model = glm::scale(model, glm::vec3(3.5f, 3.5f, 1.5f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
